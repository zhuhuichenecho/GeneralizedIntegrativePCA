...
dgbinomial1 = function(mu) 1/(mu*(1-mu))

dgbinomial2 = function(mu) (n.size)/(mu*(n.size-mu))
...
MUbinomial1 = function(theta)   exp(theta)/(1+exp(theta))

MUbinomial2 = function(theta)   n.size*exp(theta)/(1+exp(theta))
...
loglikbinomial1 = function(x,theta) sum(na.omit(x*theta-log(1+exp(theta))))

loglikbinomial2 = function(x,theta) sum(na.omit(as.vector(x*theta-n.size*log(1+exp(theta)))))

angle <- function(x,y){
  dot.prod <- x%*%y 
  theta <- acos(min(abs(svd(dot.prod)$d)))
